<template>
  <div>
    <breadcrumbs back="/store-activities" :title="$t('LABELS.store-activities')" :items="breads" />
...
function handleSubmit(values, actions) {
  ...
  let url = "store-activities";

  if (route.params.id) {
    frmData.append("_method", "PUT");
    url = `store-activities/${values.id}`;
  }
...
const breads = [
  ...
  },
  {
    ...
    path: `/store-activities/form${route.params.id ? "/" + route.params.id : ""}`,
  },
];

function getData() {
  axios.get(`store-activities/${route.params.id}`).then((res) => {
