...
function handleSubmit(values, actions) {
  ...
  frmData.append("account", values.account);
  frmData.append("bank_name", values.bank_name);
  frmData.append("iban", values.iban);
...
function getData() {
  axios.get(`bank-accounts/${route.params.id}`).then((res) => {
    const result = res.data.data;

    // initialValues.nameAr = result.name;
    //  initialValues.nameEn = result.name;
    initialValues.account = result.account;
    initialValues.bank_name = result.bank_name;
    initialValues.iban = result.iban;
